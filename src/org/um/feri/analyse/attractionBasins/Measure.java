...
public class Measure {
    ...
    /**
     * In-memory overload — accepts pre-built Fill2D objects directly,
     * skipping all disk reads. Used by CombinedTest.
     */
    public static void drawAttractionBasins(Fill2D[] fills, DoubleProblem[] problems, PipelineConfig config) throws Exception {
        for (int i = 0; i < fills.length; i++) {
            System.out.println(problems[i].getName() + " (" + (i + 1) + "/" + fills.length + ")");
            renderImages(fills[i], problems[i], config);
        }
    }
    ...
    public static void drawAttractionBasins(DoubleProblem[] problems, PipelineConfig config) throws Exception {
        for (int i = 0; i < problems.length; i++) {
            ...
            renderImages(fill2D, problems[i], config);
        }
    }

    /** Core rendering logic — shared by both overloads. */
    private static void renderImages(Fill2D fill2D, DoubleProblem problem, PipelineConfig config) {
        ...
        try {
            ...
            ig2.drawString(problem.upperLimit.get(0).toString(), 0, fontSize);
            ig2.drawString("0", 0, y / 2 + fontSize);
            ig2.drawString(problem.lowerLimit.get(0).toString(), 0, y);
            ig2.drawString(problem.lowerLimit.get(0).toString(), fontSize, y + fontSize);
            ig2.drawString("0", x / 2, y + fontSize);
            ig2.drawString(problem.upperLimit.get(0).toString(), x - fontSize, y + fontSize);
        ...
        System.out.println("Finished: " + problem.getName());
    }
